...
suspend fun fetchFullName(supabaseClient: SupabaseClient, userId: String): String? {
    // Fetching the user profile for the specified userId from the "users" table
    val existingUsers = supabaseClient.from("users")
        .select(columns = Columns.list("id, email, fullname, username"))
        .decodeList<User>()

    // Find the user with the matching ID and return their username
    return existingUsers.find { it.id == userId }?.fullname
}
